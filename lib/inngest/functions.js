  ...
  async ({ step }) => {
    ...
    for(const budget of budgets){
        ...
        await step.run(`check-budget-${budget.id}`,async()=>{
            ...
            console.log(`Checking budget ${budget.id} for user ${budget.user.email}`);

            const expenses = await db.transaction.aggregate({
                where:{
                    userId:budget.userId,
                    accountId:defaultAccount.id,//only onsider defautl account
                    type:"EXPENSE",
                    date:{
                        gte:startDate
                    },
            ...
            console.log(`Budget: ${budgetAmount}, Expenses: ${totalExpenses}, Percentage: ${percentageUsed}%`);
            console.log(`Last alert sent: ${budget.lastAlertSent}`);
            console.log(`Should send alert: ${percentageUsed >= 80 && (!budget.lastAlertSent || isNewMonth(new Date(budget.lastAlertSent),new Date()))}`);
            ...
            if(percentageUsed >= 80 && (!budget.lastAlertSent || isNewMonth(new Date(budget.lastAlertSent),new Date()))){
                ...
                const emailResult = await sendEmail({
                    ...
                    subject:`Budget Alert: ${percentageUsed.toFixed(1)}% of your budget used`,
                    react:EmailTemplate({
                ...
                console.log("Updated lastAlertSent");
            }
            ...
            return await db.transaction.findMany({
                where:{
                    ...
                    OR:[
                        ...
                        {nextRecurringDate:{lte:new Date()}},//Due date is passed
...
function isTransactionDue(transaction){
    // If nor lastProcessed date,transaction is due
